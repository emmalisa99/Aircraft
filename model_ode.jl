...
function dm2angle(angle,m)
    return cos(angle*pi/180)*m / (c_lift[2] * angle + c_lift[1]) - cos(Angle_init_rad) * aircraft.gross_mass / C_L_0
end

find_dm2angle(m) = fzero(angle->dm2angle(angle,m), 0)

function dm2AngleThrust(X)
    m = X[11]
